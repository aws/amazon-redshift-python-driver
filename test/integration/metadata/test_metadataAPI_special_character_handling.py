...
startup_stmts: typing.Tuple[str, ...] = (
    ...
    'create table {}.{} ("%cOl1" INT, "%cOl2" INT, "%cOl3" INT, "%cOl4" INT, "%cOl5" INT);'.format(
        schema_name, '"test_ Table_ 5"'
    ),
...
shutdown_stmts: typing.Tuple[str, ...] = ("DROP SCHEMA IF EXISTS {} CASCADE;".format(schema_name),)
...
@pytest.mark.parametrize(
    "catalog, schema, table_name, column_name, expected_tables_count, expected_columns_count", test_cases
)
def test_get_tables_special_character(
    db_kwargs, catalog, schema, table_name, column_name, expected_tables_count, expected_columns_count
) -> None:
    with redshift_connector.connect(**db_kwargs) as conn:
        with conn.cursor() as cursor:
            result: tuple = cursor.get_tables(catalog, schema, table_name, [])
...
def test_get_columns_special_character(
    db_kwargs, catalog, schema, table_name, column_name, expected_tables_count, expected_columns_count
) -> None:
    with redshift_connector.connect(**db_kwargs) as conn:
        with conn.cursor() as cursor:
