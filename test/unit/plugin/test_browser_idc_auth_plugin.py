...
from redshift_connector.plugin.browser_idc_auth_plugin import (
...
def test_check_required_parameters_raises_if_start_url_missing(value):
    ...
    with pytest.raises(InterfaceError,
                       match="IdC authentication failed: The start URL must be included in the connection parameters."):
...
def test_check_required_parameters_raises_if_idc_region_missing(value):
    ...
    with pytest.raises(InterfaceError,
                       match="IdC authentication failed: The IdC region must be included in the connection parameters."):
        idc_credentials_provider.get_auth_token()
...
def test_get_auth_token_fetches_idc_token(mocker):
    ...
    test_register_client_cache_key: str = f"{idc_credentials_provider.idc_client_display_name}:{idc_credentials_provider.idc_region}"
    ...
    mocker.patch.object(idc_credentials_provider, "start_device_authorization",
                        return_value=mocked_start_device_auth_result)
    mocker.patch(
        "redshift_connector.plugin.browser_idc_auth_plugin.BrowserIdcAuthPlugin.open_browser")
    ...
    idc_credentials_provider.register_client.assert_called_once_with(
        test_register_client_cache_key, idc_credentials_provider.idc_client_display_name, idc_credentials_provider.CLIENT_TYPE,
        idc_credentials_provider.IDC_SCOPE
    )
    idc_credentials_provider.start_device_authorization.assert_called_once_with(
        mocked_register_client_result['clientId'], mocked_register_client_result['clientSecret'],
