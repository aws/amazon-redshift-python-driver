...
get_catalog_result: typing.List[typing.List[str]] = [
    [current_catalog],
]

get_schema_result: typing.List[typing.List[str]] = [
    ["test_schema_1", current_catalog],
    ["test_schema_2", current_catalog],
]

get_table_result: typing.List[typing.List[typing.Union[str, None]]] = [
...
get_column_result: typing.List[typing.List[str]] = [
...
def get_tables_matches(catalog, schema, table, table_type) -> typing.List[typing.List]:
    ...
    for row in get_table_result:
        if (matches_catalog(catalog, row[0]) and matches(schema, row[1]) and matches(table, row[2])) and (
            table_type is None or len(table_type) == 0 or row[3] in table_type
        ):
            result.append(row)
    return result
...
def get_columns_matches(catalog, schema, table, column) -> typing.List[typing.List]:
    ...
    for row in get_column_result:
        if (
            matches_catalog(catalog, row[0])
            and matches(schema, row[1])
            and matches(table, row[2])
            and matches(column, row[3])
        ):
            result.append(row)
    return result


def matches(str1: typing.Optional[str], str2: typing.Union[str, None]) -> bool:
    if str2 is None:
        return str1 is None
    return (
        str1 is None
        ...
        or str1 == "%"
        or str1 == str2
        or (str1 is not None and "%" in str1 and str2.startswith("test"))
    )


def matches_catalog(str1: typing.Optional[str], str2: typing.Union[str, None]) -> bool:
    if str2 is None:
        return str1 is None
