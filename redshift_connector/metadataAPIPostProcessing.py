...
class MetadataAPIPostProcessing(MetadataAPIHelper):
    ...
    def get_catalog_post_processing(self, intermediate_rs: typing.Tuple) -> typing.Tuple:
        ...
        if self._cursor._SHOW_DATABASES_Col_index is not None:
            for row in intermediate_rs:
                ...
                final_row: typing.List = [row[db_name_index]]
        ...
        if self._cursor._SHOW_SCHEMAS_Col_index is not None:
            for rs in intermediate_rs:
                for row in rs:
                    ...
                    db_name_index = self._cursor._SHOW_SCHEMAS_Col_index.get(self._SHOW_SCHEMA_database_name)
                    if schema_name_index is None or db_name_index is None:
                        raise InterfaceError("Required keys not found in _SHOW_SCHEMAS_Col_index")
                    final_row: typing.List = [
                        row[schema_name_index],
                        row[db_name_index],
                    ]
    ...
    def get_table_post_processing(self, intermediate_rs: typing.List[typing.Tuple], types: list) -> typing.Tuple:
        ...
        if self._cursor._SHOW_TABLES_Col_index is not None:
            for rs in intermediate_rs:
                for row in rs:
                    table_type_index = self._cursor._SHOW_TABLES_Col_index.get(self._SHOW_TABLES_table_type)
                    db_name_index = self._cursor._SHOW_TABLES_Col_index.get(self._SHOW_TABLES_database_name)
                    schema_name_index = self._cursor._SHOW_TABLES_Col_index.get(self._SHOW_TABLES_schema_name)
                    table_name_index = self._cursor._SHOW_TABLES_Col_index.get(self._SHOW_TABLES_table_name)
                    remarks_index = self._cursor._SHOW_TABLES_Col_index.get(self._SHOW_TABLES_remarks)

                    if None in (table_type_index, db_name_index, schema_name_index, table_name_index, remarks_index):
                        raise InterfaceError("Required keys not found in _SHOW_TABLES_Col_index")

                    if types is None or len(types) == 0 or row[table_type_index] in types:
                        final_row: typing.List = [
                            row[db_name_index],
                            row[schema_name_index],
                            row[table_name_index],
                            row[table_type_index],
                            row[remarks_index],
                            "",
                            "",
                            "",
